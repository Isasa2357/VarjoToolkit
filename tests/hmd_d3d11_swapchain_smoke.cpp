    ...
    int32_t imageIndex)
{
    ...
    VTK_HMD_TEST_REQUIRE(
        imageIndex < swapChain.config().numberOfTextures);
    ...
    VTK_HMD_TEST_REQUIRE(
        textureDesc.Width == static_cast<UINT>(config.textureWidth));
    VTK_HMD_TEST_REQUIRE(
        textureDesc.Height == static_cast<UINT>(config.textureHeight));
    VTK_HMD_TEST_REQUIRE(
        textureDesc.ArraySize >= static_cast<UINT>(config.textureArraySize));
    ...
    for (int32_t arrayIndex = 0;
        ...
        vtk_hmd_test::throwIfFailed(
            device->CreateRenderTargetView(d3dTexture, &rtvDesc, &rtv),
...
void runD3D11SwapChainSmokeTest()
{
    ...
    auto device = vtk_hmd_test::createD3D11DeviceForVarjo(
        session.get(),
        context);
    VTK_HMD_TEST_REQUIRE(device != nullptr);
    VTK_HMD_TEST_REQUIRE(context != nullptr);
    ...
    auto config = vtk_hmd_test::makeSmokeColorSwapChainConfig(
        session.get(),
        frameInfo);
    VarjoSwapChain swapChain = VarjoSwapChain::createD3D11(
        session.shared(),
        device.Get(),
        config);
