...
public:
    ...
    void initialize(varjo_Session* session)
    {
        ...
        vtk_hmd_test::throwIfFailed(
            device_->CreateCommandQueue(&queueDesc, IID_PPV_ARGS(&queue_)),
            "CreateCommandQueue");
        vtk_hmd_test::throwIfFailed(
        ...
        int32_t imageIndex)
    {
        ...
        VTK_HMD_TEST_REQUIRE(
            resourceDesc.Width == static_cast<UINT64>(config.textureWidth));
        VTK_HMD_TEST_REQUIRE(
            resourceDesc.Height == static_cast<UINT>(config.textureHeight));
        VTK_HMD_TEST_REQUIRE(
            resourceDesc.DepthOrArraySize >=
            static_cast<UINT16>(config.textureArraySize));
        ...
        for (int32_t arrayIndex = 0;
            ...
            rtvDesc.Texture2DArray.FirstArraySlice =
                static_cast<UINT>(arrayIndex);
            rtvDesc.Texture2DArray.ArraySize = 1;
            device_->CreateRenderTargetView(
                resource,
                &rtvDesc,
                handleFor(heapStart, descriptorSize, arrayIndex));
        }

        vtk_hmd_test::throwIfFailed(allocator_->Reset(), "Allocator Reset");
        vtk_hmd_test::throwIfFailed(
        ...
        auto toCommon = transitionBarrier(
            resource,
            D3D12_RESOURCE_STATE_RENDER_TARGET,
            D3D12_RESOURCE_STATE_COMMON);
...
private:
    static D3D12_CPU_DESCRIPTOR_HANDLE handleFor(
        D3D12_CPU_DESCRIPTOR_HANDLE start,
        UINT descriptorSize,
        int32_t index)
    {
        start.ptr += static_cast<SIZE_T>(descriptorSize) *
    ...
    void waitForGpu()
    {
        ...
        vtk_hmd_test::throwIfFailed(
            queue_->Signal(fence_.Get(), waitValue),
            "Queue Signal");
        if (fence_->GetCompletedValue() < waitValue) {
            vtk_hmd_test::throwIfFailed(
                fence_->SetEventOnCompletion(waitValue, fence_event_),
                "SetEventOnCompletion");
            VTK_HMD_TEST_REQUIRE(
                WaitForSingleObject(fence_event_, INFINITE) == WAIT_OBJECT_0);
        }
    }
...
void runD3D12SwapChainSmokeTest()
{
    ...
    auto config = vtk_hmd_test::makeSmokeColorSwapChainConfig(
        session.get(),
        frameInfo);
    VarjoSwapChain swapChain = VarjoSwapChain::createD3D12(
        session.shared(),
        context.commandQueue(),
        config);
    VTK_HMD_TEST_REQUIRE_MESSAGE(
        swapChain.valid(),
        swapChain.lastError());
    ...
    VTK_HMD_TEST_REQUIRE(
        layerFrame.end(layer, frameInfo.frameNumber()));
}
