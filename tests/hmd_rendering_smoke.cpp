...
void runRenderingSmokeTest()
{
    ...
    std::cout << "Rendering smoke viewCount=" << viewCount
    ...
    VTK_HMD_TEST_REQUIRE(
        (layer.header()->flags & varjo_LayerFlag_DepthTesting) != 0);
    ...
    for (int32_t viewIndex = 0; viewIndex < viewCount; ++viewIndex) {
        ...
        VTK_HMD_TEST_REQUIRE(
            layerView.projection.value[0] == view.projectionMatrix[0]);
        VTK_HMD_TEST_REQUIRE(
            layerView.view.value[0] == view.viewMatrix[0]);
    }
    ...
    const auto submitInfo = VarjoLayerFrame::makeSubmitInfo(
        layers,
    ...
    VarjoLayerFrame invalidLayerFrame(
        static_cast<varjo_Session*>(nullptr));
    ...
    VTK_HMD_TEST_REQUIRE(
        !invalidLayerFrame.endEmpty(frameInfo.frameNumber()));
}
