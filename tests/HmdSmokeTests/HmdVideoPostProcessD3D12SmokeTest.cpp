...
int main()
{
    std::cout << "VarjoToolkit HMD D3D12 video post process smoke test\n";
    ...
    try {
        ...
        if (!d3d12.queue) {
            return hmdFail("D3D12 command queue is null");
        }
        ...
        if (!shader.configureD3D12(
                d3d12.queue.Get(),
                config,
                shaderBlob->GetBufferPointer(),
                static_cast<int32_t>(shaderBlob->GetBufferSize()))) {
            return hmdFail(std::string("configureD3D12 failed: ") + shader.lastError());
        }
        ...
        std::cout << "[PASS] HMD D3D12 video post process smoke test passed\n";
        return 0;
