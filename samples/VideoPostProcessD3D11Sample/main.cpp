...
void printUsage()
{
    std::cout
        ...
        << "\n"
        << "Usage:\n"
        << "  VarjoVideoPostProcessD3D11Sample.exe [options]\n"
        << "\n"
        << "Options:\n"
...
ComPtr<ID3D11Device> createD3D11DeviceForVarjo(varjo_Session* session)
{
...
ComPtr<ID3DBlob> compileShader(const char* source)
{
    ...
    const HRESULT hr = D3DCompile(
        source,
        std::strlen(source),
        "VarjoVideoPostProcessD3D11Sample.hlsl",
        nullptr,
        nullptr,
        "main",
        "cs_5_0",
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        std::cout << "Creating D3D11 device for Varjo adapter...\n";
        ComPtr<ID3D11Device> device = createD3D11DeviceForVarjo(session.get());
        ...
        if (!postProcess.configureD3D11(
                device.Get(),
                config,
                shaderBlob->GetBufferPointer(),
                static_cast<int32_t>(shaderBlob->GetBufferSize()))) {
            std::cerr << "configureD3D11 failed: " << postProcess.lastError() << "\n";
            return 1;
        }
        ...
        std::cout << "Video post process enabled. Press Ctrl+C to stop.\n";
